...
const val Input2 = "1224\t926\t1380\t688\t845\t109\t118\t88\t1275\t1306\t91\t796\t102\t1361\t27\t995\n" +
...
fun firstPart(): Int {
    ...
    val matrix = rows.mapTo(mutableListOf()) { row -> row.split("\t").filter { !it.isEmpty() }.map { Integer.parseInt(it) } }
    val differences = matrix.map { row -> row.max()!! - row.min()!! }
    return differences.sum()
}

fun secondPart(): Int {
    val rows = Input2.split("\n").filter { !it.isEmpty() }
    val matrix = rows.mapTo(mutableListOf()) { row -> row.split("\t").filter { !it.isEmpty() }.map { Integer.parseInt(it) }.sortedDescending() }
    var sum = 0
    for (row in matrix)
    {
        for ((index,numerator) in row.withIndex())
        {

            row.drop(index + 1)
                    .asSequence()
                    .filter { numerator.rem(it) == 0 }
                    .forEach { sum += numerator.div(it) }
        }
    }
return sum
}
